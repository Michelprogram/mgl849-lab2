...
void *task_decision(void *arg)
{
    ...
    while (1) {
        ...
        gas_parsed_t current = args->shared->income;
        pthread_mutex_unlock(&args->shared->lock);
 
        if (current.gas == GAS_INVALID) continue;
        ...
        if (create_alert_cmd(idx, current.alarm, state[idx].last_alarm,
                             cmd, sizeof(cmd))) {
        ...
        if (choose_action(idx, current.alarm, &state[idx],
                          cmd, sizeof(cmd))) {
        ...
        printf("[decision] Gas%d | alarm=%d | active_type=%d level=%d\n",
               current.gas, current.alarm,
               state[idx].active_type, state[idx].active_level);
    }
